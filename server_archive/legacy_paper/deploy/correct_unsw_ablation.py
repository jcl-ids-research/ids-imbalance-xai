...
def rebalance(ddpm_models: dict[int, MLPDDPM], X: np.ndarray, y: np.ndarray,
    ...
    undersampled and minority classes are topped up by the DDPM. The total
    training size is preserved, which keeps the ablation contrast about
    *balancing* rather than about data volume.

    Returns (X_balanced, y_balanced, report) where report records the class
    distribution before and after - this is the evidence the editor asked for.
    """
    ...
    for cls, n_real in zip(classes.tolist(), counts.tolist()):
        ...
        if n_real >= target:
            keep = rng.choice(n_real, size=target, replace=False)
            X_parts.append(real[keep])
            y_parts.append(np.full(target, cls, dtype=np.int64))
            actions[str(cls)] = f"undersampled {n_real} -> {target}"
            after[str(cls)] = target
            continue
        ...
        y_parts.append(np.full(target, cls, dtype=np.int64))
        actions[str(cls)] = f"augmented {n_real} + {n_syn} synthetic -> {target}"
        after[str(cls)] = target
    ...
    report = {
        ...
        "imbalance_ratio_before": float(counts.max() / counts.min()),
        "imbalance_ratio_after": 1.0,
    }
    print(f"    [BAL] target={target}/class  before={before}  after={after}", flush=True)
    return X_bal[perm], y_bal[perm], report
...
def run_seed(seed: int, data_dir: str, out_dir: str, smoke: bool = False) -> dict:
    ...
    result = {
        "seed": seed,
        "protocol": "official-split + conditional-DDPM balancing + paired-init + semantic-views + " + FUSION_MODE + "-fusion",
